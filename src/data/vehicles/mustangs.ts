import { Mustang } from "../../types/Vehicle";

export const mustangs: Mustang[] = [

  // ===============================
  //  2025 ECOBOOST
  // ===============================
  
  {
    id: "2025-ecoboost-fastback",
    year: 2025,
    trim: "Ecoboost",
    bodyStyle: "Fastback",

    engine: {
      value: "2.3L Turbo Inline-4",
      definition: "A turbocharged 4-cylinder engine that balances power and efficiency.",
      analogy: "A smaller engine with a turbo is like a compact athlete with a boost button.",
      buyerValue: "Good performance with better fuel economy and lower cost."
    },

    liters: {
      value: "2.3",
      definition: "Engine displacement size.",
      analogy: "Lung size - smaller but boosted.",
      buyerValue: "Smaller engines are more efficient but can still be powerful with turbo."
    },

    cylinders: {
      value: 4,
      definition: "Number of combustion chambers.",
      analogy: "4 workers doing efficient, focused work.",
      buyerValue: "Lower fuel use, lighter feel."
    },

    horsepower: {
      value: "315",
      definition: "Total engine output.",
      analogy: "Sprint speed.",
      buyerValue: "Quick acceleration for daily driving."
    },

    torque: {
      value: "350",
      definition: "Rotational force.",
      analogy: "Initial shove when you hit the gas.",
      buyerValue: "Feels quick off the line."
    },

    transmission: {
      value: "10-speed automatic",
      definition: "Gear system controlling power delivery.",
      analogy: "Bike gears shifting automaitcally.",
      buyerValue: "Smooth and easy driving."
    },

    drivetrain: {
      value: "RWD",
      definition: "Rear-wheel drive.",
      analogy: "Push from behind.",
      buyerValue: "Sporty handling feel."
    },
    
    keyFeatures: [
      "Digital gauge cluster",
      "Large touchscreen",
      "Drive modes",
      "Selectable steering"
    ]
  },

  // ===================================
  // 2025 GT
  // ===================================

  {
    id: "2025-gt-fastback",
    year: 2025,
    trim: "GT",
    bodyStyle: "Fastback",

    engine: {
      value: "5.0L V8",
      definition: "Naturally aspirated 8-cylinder engine focused on power and sound.",
      analogy: "A full orchestra vs a solo instrument.",
      buyerValue: "Classic Mustang feel, sound, and strong performance."
    },

    liters: {
      value: 5.0,
      definition: "Engine size.",
      analogy: "Big lungs with deep breathing.",
      buyerValue: "More natural power and stronger presence."
    },
      
    cylinders: {
      value: 8,
      definition: "Number of power chambers.",
      analogy: "8 workers working together smoothly.",
      buyerValue: "Smoother, louder, more powerful."
    },
    
    horsepower: {
      value: 480,
      definition: "Engine output.",
      analogy: "Top-end speed potential.",
      buyerValue: "Strong acceleration and highway power."
    },

    torque: {
      value: 415,
      definition: "Rotational force.",
      analogy: "Push that throws you back in the seat.",
      buyerValue: "Powerful launches and feel."
    },

    transmission: {
      value: "6-speed manual or 10-speed automatic",
      definition: "Manual or automatic gear control.",
      analogy: "Manual = playing guitar, Auto = pressing play.",
      buyerValue: "Choice between engagement and convenience."
    },

    drivetrain: {
      value: "RWD",
      definition: "Rear-wheel drive.",
      analogy: "Push from behind.",
      buyerValue: "Classic sports car handling."
    },

    keyFeatures: [
      "Active exhaust",
      "Performance gauges",
      "Launch control",
      "Track apps"
    ]
  },

  // ==========================
  // 2025 Dark Horse
  // ==========================
  
  {
    id: "2025-darkhorse-fastback",
    year: 2025,
    trim: "Dark Horse",
    bodyStyle: "Fastback",

    engine: {
      value: "5.0 V8 (High Performance)",
      definition: "Upgraded V8 tuned for track performance.",
      analogy: "Race-prepped version of the same engine.",
      buyerValue: "More aggresive power and track capability."
    },

    liters: {
      value: 5.0,
      definition: "Engine displacement.",
      analogy: "Same lungs, trained athlete.",
      buyerValue: "More refined performance tuning."
    },

    cylinders: {
      value: 8,
      definition: "Number of combustion chambers.",
      analogy: "Elite team of workers.",
      buyerValue: "Smooth and powerful delivery."
    },

    horsepower: {
      value: 500,
      definition: "Engine output.",
      analogy: "Elite sprint athlete.",
      buyerValue: "Top-tier performance."
    },

    torque: {
      value: 418,
      definition: "Rotational force.",
      analogy: "Explosive push.",
      buyerValue: "Aggresive acceleration."
    },

    transmission: {
      value: "6-speed manual or 10-speed automatic",
      definition: "Performance-tuned gear systems.",
      analogy: "Precision gear control.",
      buyerValue: "Better control at high speeds."
    },

    drivetrain: {
      value: "RWD",
      definition: "Rear-wheel drive.",
      analogy: "Push-focused performance.",
      buyerValue: "Track-ready handling."
    },

    keyFeatures: [
      "",
      "",
      "",
      ""
    ]
  }

];